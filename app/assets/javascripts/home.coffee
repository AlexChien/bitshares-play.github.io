...
get_latest_downloads = ->
  ...
  .done (data) ->
    ...
    for i in [0...platformCount]
      ...
      dlDiv = $("<div class='col-md-#{colCls}'><img class='platform-icon' src='/assets/icon-#{platform}.svg' /><h3>#{d.platform}</h3><p style='font-size:10px;'>sha1hash: #{d.sha1hash}<br />Size: #{d.fileSize}</p><a class='btn btn#{i} withripple' href='#{d.url}' role='button' data-platform='#{d.platform}' data-version='#{data.version}'><img src='/assets/ic_cloud_download_48px.svg' /><div class='ripple-wrapper'></div></a></div>")
...
get_latest_downloads2 = ->
  ...
  .done (data) ->
    ...
    for i in [0...platformCount]
      d = data.downloads[i]
      platform = d.platform.split(' ')[0].toLowerCase()
      dlDiv = $("<div class='col-md-#{colCls} animated fadeInUp'><img class='platform-icon' src='/assets/icon-#{platform}.svg' /><h3>#{d.platform}</h3><p style='font-size:10px;'>sha1hash: #{d.sha1hash}<br />Size: #{d.fileSize}</p><a class='btn btn#{i} withripple' href='#{d.url}' role='button' data-platform='#{d.platform}' data-version='#{data.version}'><img src='/assets/ic_cloud_download_48px.svg' /><div class='ripple-wrapper'></div></a></div>")
...
(->
  ...
  # bg image
  # $.backstretch("/assets/soldierf.jpg");
  # $.backstretch("/assets/soldierh.jpg");
